...
AlchemyCodeBinary = Annotated[
    list[int], Field(min_length=8, max_length=8), Field(ge=0, le=63)
]
...
class Item(SQLModel, table=True):
    ...
    code: AlchemyCode = Field(
        default_factory=generate_alchemy_code, description="8 character alchemy code"
    )
    components: str = Field(
        default="", description="Chain of components used to create this item"
    )
