...
async def generate_aspect_context(titles: list[Title]) -> str:
    ...
    for title in titles:
        ...
        async with aiofiles.open(f"{PROMPTS_DIRECTORY}/aspects/{title.title_aspect.lower()}.md") as f:
...
async def create_fraymotif(titles: list[Title], memory: str, additional_info: str) -> Fraymotif:
    ...
    async with aiofiles.open(f"{PROMPTS_DIRECTORY}/fraymotifs/fraymotif_generator.md") as f:
